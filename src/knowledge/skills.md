...
**One-line summary:** Backend-leaning full-stack developer skilled in Node.js/Express APIs, with applied experience in RAG, vector search, and LLM integration as he builds toward AI engineering specialization. ~2 years into a self-taught engineering journey, validated through freelance client work and production-style personal projects.
...
*Proof: Project Camp (RBAC + JWT + REDIS caching), VideoTube (refresh rotation + BullMQ workers + REDIS Caching), OrgMemory (JWT + Zod validation + Hybrid Search).*
...
### AI Engineering (growing specialization)
Hands-on experience building Retrieval-Augmented Generation (RAG) from scratch without LangChain — including hybrid dense + BM25/keyword retrieval, query expansion, re-ranking, MMR diversification, and confidence-gated hallucination prevention. Embedding models and vector search (Qdrant, Supabase pgvector, Xenova Transformers). LLM integration via Groq (Whisper transcription + inference) and OpenRouter/OpenAI. Prompt engineering for production RAG pipelines.
...
### Currently Deepening
TypeScript, Redis, BullMQ, Docker, advanced system design, agentic AI systems, distributed architectures, DevOps practices.
